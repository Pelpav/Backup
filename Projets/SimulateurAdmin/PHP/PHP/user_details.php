...
function calculerMensualite($montantDuPret, $tauxInteretAnnuel, $dureeDuPretEnAnnees)
{
    ...
    $nombreTotalDePaiements = $dureeDuPretEnAnnees * 12;

    // Calculer la mensualité en utilisant la formule du remboursement d'un prêt à mensualités constantes
    $mensualite = ($montantDuPret * $tauxInteretMensuel) / (1 - pow(1 + $tauxInteretMensuel, -$nombreTotalDePaiements));

    // Arrondir la mensualité au chiffre entier le plus proche
    $mensualiteArrondie = round($mensualite, 2);

    return $mensualiteArrondie;
}
        ...
        <div class="loan-summary mt-4">
            ...
            <table class="table">
                ...
                <tbody>
                    ...
                    if ($duree <= 2) {
                        $taux = 8;
                    } elseif ($duree > 2 && $duree <= 4) {
                        $taux = 10;
                    } elseif ($duree > 4 && $duree <= 5) {
                        $taux = 12;
                    }
                    ...
                    $mensualite = calculerMensualite($userData['mt_pret'], $taux, $duree);
                    for ($mois = 1; $mois <= $duree * 12; $mois++) {
                        ...
                        // Éviter des mensualités nulles aux derniers mois
                        if ($montantRestant > 0) {
                            // Si la mensualité est supérieure au montant restant, ajuster la mensualité
            ...
            <td>$montantRestant FCFA</td>
                            ...
                            // Sortir de la boucle si le montant restant est nul
                            break;
                        }
                    }
